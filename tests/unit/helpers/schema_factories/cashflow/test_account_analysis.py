...
from tests.helpers.schema_factories.cashflow.account_analysis import (
...
def test_create_cross_account_analysis_schema():
    ...
    assert schema.correlations["1-2"]["correlation"].correlation_score == Decimal("0.75")
...
def test_create_cross_account_analysis_schema_with_custom_values():
    ...
    balance_dist = create_balance_distribution_schema(
        account_id=3,
        average_balance=Decimal("5000.00")
    ).model_dump()
    ...
    risk_assessment_data = create_account_risk_assessment_schema(
        account_id=3,
        overall_risk_score=Decimal("0.05")
    ).model_dump()
