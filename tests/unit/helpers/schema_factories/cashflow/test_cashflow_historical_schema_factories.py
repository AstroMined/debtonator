...
from tests.helpers.schema_factories.cashflow.historical import (
...
def test_create_seasonality_analysis_schema():
    ...
    assert sum(schema.monthly_patterns.values()).quantize(Decimal("0.01")) == Decimal("1.00")
    assert sum(schema.day_of_week_patterns.values()).quantize(Decimal("0.01")) == Decimal("1.00")
    
    # Allow a small tolerance for day_of_month_patterns since they may not sum exactly to 1.0
    day_of_month_sum = sum(schema.day_of_month_patterns.values()).quantize(Decimal("0.01"))
