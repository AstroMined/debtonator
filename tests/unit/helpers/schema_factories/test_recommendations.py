...
from tests.helpers.schema_factories.recommendations import (
...
def test_create_impact_metrics_schema_with_partial_custom_values():
    ...
    schema = create_impact_metrics_schema(
        ...
        credit_utilization_impact=Decimal("0.1")
    )
...
def test_create_bill_payment_timing_recommendation_schema():
    ...
    assert schema.current_due_date > schema.recommended_date  # Due date is after recommended date
...
def test_create_bill_payment_timing_recommendation_schema_with_custom_values():
    ...
    custom_impact = create_impact_metrics_schema(
        balance_impact=Decimal("-300.00"),
        savings_potential=Decimal("100.00")
    )
...
def test_create_recommendation_response_schema():
    ...
    expected_savings = sum(
        r.impact.savings_potential for r in schema.recommendations if r.impact.savings_potential is not None
    )
