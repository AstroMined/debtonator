...
def test_create_source_statistics_schema():
    ...
    assert schema.total_amount == MEDIUM_AMOUNT * Decimal("20") * Decimal(
        "24"
    )  # 48000.00
    assert schema.average_amount == MEDIUM_AMOUNT * Decimal("20")  # 2000.00
    assert schema.min_amount == MEDIUM_AMOUNT * Decimal("20") - MEDIUM_AMOUNT * Decimal(
        "1.5"
    )  # 1850.00
    assert schema.max_amount == MEDIUM_AMOUNT * Decimal("20") + MEDIUM_AMOUNT * Decimal(
        "2"
    )  # 2200.00
...
def test_create_income_trends_analysis_schema_with_custom_values():
    ...
    source_statistics = [
        create_source_statistics_schema(
            source="Main Job", total_occurrences=26, average_amount=Decimal("2500.00")
        ).model_dump(),
        create_source_statistics_schema(
            source="Side Project",
            total_occurrences=12,
    ...
    seasonality = create_seasonality_metrics_schema(
        period=PeriodType.QUARTERLY, confidence_score=Decimal("0.9")
    ).model_dump()
...
def test_create_income_trends_request_schema_with_partial_fields():
    ...
    schema = create_income_trends_request_schema(
        start_date=start_date, source="Freelance"
    )
