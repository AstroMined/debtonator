...
async def test_bills_relationship(db_session: AsyncSession, test_category: Category):
...
async def test_lazy_loading_config(db_session: AsyncSession, test_category: Category):
    """Test lazy loading configuration of relationships"""
    # Create child category
    child = Category(name="Child Category", parent_id=test_category.id)
...
async def test_create_hierarchical_categories(db_session: AsyncSession):
    ...
    parent = Category(name="Parent Category", description="Parent Description")
    db_session.add(parent)
    await db_session.commit()

    child = Category(
        name="Child Category", description="Child Description", parent_id=parent.id
    )
    db_session.add(child)
    await db_session.commit()
...
async def test_category_hierarchical_relationships(db_session: AsyncSession):
    ...
    parent = Category(name="Parent", parent_id=grandparent.id)
    db_session.add(parent)
    await db_session.commit()
    ...
    stmt = (
        select(Category)
        .where(Category.id == child.id)
        .options(selectinload(Category.parent.of_type(Category)))
    )
    result = await db_session.execute(stmt)
    child = result.unique().scalar_one()
