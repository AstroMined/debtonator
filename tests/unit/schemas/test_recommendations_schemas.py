...
def test_impact_metrics_optional_fields():
    ...
    data = ImpactMetrics(balance_impact=Decimal("-50.00"), risk_score=Decimal("25.5"))
...
def test_bill_payment_timing_recommendation_valid():
    ...
    assert (
        data.reason
        == "Paying earlier will avoid potential late fees and improve cash flow"
    )
...
def test_enum_validation():
    ...
    with pytest.raises(
        ValidationError,
        match="Input should be 'bill_payment_timing', 'account_selection', 'split_distribution' or 'credit_utilization'",
    ):
        RecommendationBase(
    ...
    with pytest.raises(
        ValidationError, match="Input should be 'high', 'medium' or 'low'"
    ):
        RecommendationBase(
            type=RecommendationType.BILL_PAYMENT_TIMING,
...
def test_required_fields():
    ...
    with pytest.raises(ValidationError, match="Field required"):
        ImpactMetrics(
            credit_utilization_impact=Decimal("5.00"), risk_score=Decimal("25.5")
        )

    with pytest.raises(ValidationError, match="Field required"):
        ImpactMetrics(
            balance_impact=Decimal("-50.00"), credit_utilization_impact=Decimal("5.00")
        )
    ...
    with pytest.raises(ValidationError, match="Field required"):
        RecommendationBase(confidence=ConfidenceLevel.HIGH, impact=impact)

    with pytest.raises(ValidationError, match="Field required"):
        RecommendationBase(type=RecommendationType.BILL_PAYMENT_TIMING, impact=impact)

    with pytest.raises(ValidationError, match="Field required"):
        RecommendationBase(
            type=RecommendationType.BILL_PAYMENT_TIMING, confidence=ConfidenceLevel.HIGH
        )
    ...
    with pytest.raises(ValidationError, match="Field required"):
        RecommendationResponse(
            total_savings_potential=Decimal("75.50"), average_confidence=Decimal("0.85")
        )
