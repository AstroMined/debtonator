...
def test_required_fields():
    ...
    with pytest.raises(ValidationError, match="Field required"):
        DepositScheduleBase(
            account_id=2, schedule_date=now, amount=Decimal("500.00"), status="pending"
        )

    # Test missing account_id
    with pytest.raises(ValidationError, match="Field required"):
        DepositScheduleBase(
            income_id=1, schedule_date=now, amount=Decimal("500.00"), status="pending"
        )

    # Test missing schedule_date
    with pytest.raises(ValidationError, match="Field required"):
        DepositScheduleBase(
            income_id=1, account_id=2, amount=Decimal("500.00"), status="pending"
        )

    # Test missing amount
    with pytest.raises(ValidationError, match="Field required"):
        DepositScheduleBase(
            income_id=1, account_id=2, schedule_date=now, status="pending"
        )

    # Test missing status
    with pytest.raises(ValidationError, match="Field required"):
        DepositScheduleBase(
            income_id=1, account_id=2, schedule_date=now, amount=Decimal("500.00")
        )
...
def test_recurrence_pattern():
    ...
    simple_pattern = {"frequency": "monthly", "day": 15}
