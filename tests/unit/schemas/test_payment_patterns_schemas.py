...
def test_frequency_metrics_valid():
    ...
    data = FrequencyMetrics(
        average_days_between=30.5, std_dev_days=1.2, min_days=29, max_days=32
    )
...
def test_seasonal_metrics_valid():
    ...
    data = SeasonalMetrics(avg_days_before_due=5.3, std_dev_days=1.8, sample_size=10)
...
def test_payment_pattern_analysis_valid():
    ...
    seasonal_metrics = {
        1: SeasonalMetrics(avg_days_before_due=5.3, std_dev_days=1.8, sample_size=5),
        7: SeasonalMetrics(avg_days_before_due=4.8, std_dev_days=1.5, sample_size=5),
    }
...
def test_enum_validation():
    ...
    with pytest.raises(
        ValidationError,
        match="Input should be 'regular', 'irregular', 'seasonal' or 'unknown'",
    ):
        PaymentPatternAnalysis(
...
def test_string_length_validation():
    ...
    with pytest.raises(
        ...
        PaymentPatternRequest(category_id="X" * 101)  # Too long
...
def test_required_fields():
    ...
    with pytest.raises(ValidationError, match="Field required"):
        FrequencyMetrics(std_dev_days=1.2, min_days=29, max_days=32)
    ...
    with pytest.raises(ValidationError, match="Field required"):
        SeasonalMetrics(std_dev_days=1.8, sample_size=10)
