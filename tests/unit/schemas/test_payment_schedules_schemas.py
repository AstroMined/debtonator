...
def test_payment_schedule_base_minimal():
    ...
    data = PaymentScheduleBase(
        liability_id=1, account_id=2, scheduled_date=now, amount=Decimal("150.75")
    )
...
def test_required_fields():
    ...
    with pytest.raises(ValidationError, match="Field required"):
        PaymentScheduleBase(account_id=2, scheduled_date=now, amount=Decimal("150.75"))

    # Test missing account_id
    with pytest.raises(ValidationError, match="Field required"):
        PaymentScheduleBase(
            liability_id=1, scheduled_date=now, amount=Decimal("150.75")
        )

    # Test missing scheduled_date
    with pytest.raises(ValidationError, match="Field required"):
        PaymentScheduleBase(liability_id=1, account_id=2, amount=Decimal("150.75"))

    # Test missing amount
    with pytest.raises(ValidationError, match="Field required"):
        PaymentScheduleBase(liability_id=1, account_id=2, scheduled_date=now)
