...
def test_round_for_calculation():
    ...
    # Test exact values
    assert DecimalPrecision.round_for_calculation(Decimal("10.0000")) == Decimal(
        "10.0000"
    )
    assert DecimalPrecision.round_for_calculation(Decimal("10.5000")) == Decimal(
        "10.5000"
    )

    # Test rounding up
    assert DecimalPrecision.round_for_calculation(Decimal("10.55555")) == Decimal(
        "10.5556"
    )
    assert DecimalPrecision.round_for_calculation(Decimal("10.99995")) == Decimal(
        "11.0000"
    )
    ...
    # Test rounding down
    assert DecimalPrecision.round_for_calculation(Decimal("10.55554")) == Decimal(
        "10.5555"
    )
    assert DecimalPrecision.round_for_calculation(Decimal("10.99994")) == Decimal(
        "10.9999"
    )
...
def test_distribute_by_percentage_with_rounding():
    ...
    assert result[0] == Decimal("61.72") or result[0] == Decimal(
        "61.73"
    )  # ~50% of 123.45
    assert result[1] == Decimal("37.04") or result[1] == Decimal(
        "37.03"
    )  # ~30% of 123.45
    assert result[2] == Decimal("24.69") or result[2] == Decimal(
        "24.68"
    )  # ~20% of 123.45
...
def test_validate_sum_equals_total():
    ...
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items, Decimal("60.05"), epsilon=custom_epsilon
        )
        is True
    )
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items, Decimal("59.95"), epsilon=custom_epsilon
        )
        is True
    )
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items, Decimal("60.2"), epsilon=custom_epsilon
        )
        is False
    )
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items, Decimal("59.8"), epsilon=custom_epsilon
        )
        is False
    )
    ...
    items_custom_obj = [
        CustomItem(Decimal("10")),
        CustomItem(Decimal("20")),
        CustomItem(Decimal("30")),
    ]
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items_custom_obj, Decimal("60"), amount_attr="value"
        )
        is True
    )
    ...
    assert (
        DecimalPrecision.validate_sum_equals_total(
            items_custom_dict, Decimal("60"), amount_attr="value"
        )
        is True
    )
    ...
    assert DecimalPrecision.validate_sum_equals_total([], Decimal("100.00")) is False
