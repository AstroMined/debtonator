...
class TestAccountService:
    ...
    async def test_update_statement_balance(self, db_session):
        ...
        statement_datetime = datetime.combine(
    ...
    async def test_validate_transaction(self, db_session):
        ...
        is_valid, error = await service.validate_transaction(
            account, {"amount": Decimal("500.00"), "transaction_type": "debit"}
        )
        ...
        is_valid, error = await service.validate_transaction(
            account, {"amount": Decimal("1500.00"), "transaction_type": "debit"}
        )
    ...
    async def test_validate_credit_transaction(self, db_session):
        ...
        # Test valid debit transaction
        is_valid, error = await service.validate_transaction(
            account, {"amount": Decimal("3000.00"), "transaction_type": "debit"}
        )
        assert is_valid is True
        assert error is None
        ...
        is_valid, error = await service.validate_transaction(
            account, {"amount": Decimal("6000.00"), "transaction_type": "debit"}
        )
        assert is_valid is False
