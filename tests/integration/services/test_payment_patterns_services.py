...
from src.schemas.payment_patterns import PatternType, PaymentPatternRequest
...
async def test_analyze_regular_pattern(
    payment_pattern_service: BillPaymentPatternService, regular_payments, test_accounts
):
    request = PaymentPatternRequest(
        account_id=test_accounts[0].id, category_id="utilities"
    )
...
async def test_analyze_seasonal_pattern(
    payment_pattern_service: BillPaymentPatternService, seasonal_payments, test_accounts
):
    request = PaymentPatternRequest(
        account_id=test_accounts[0].id, category_id="utilities"
    )
    ...
    assert (
        analysis.sample_size == 6
    )  # Fixture creates 6 monthly payments with increasing amounts
    assert any(
        "seasonal" in note.lower() or "monthly" in note.lower()
...
async def test_minimal_sample_regular_pattern(
    ...
    )  # Lower confidence due to minimal sample size
    assert analysis.sample_size == 3
    assert abs(analysis.frequency_metrics.average_days_between - 30) < 1


async def test_gapped_payment_pattern(
    payment_pattern_service: BillPaymentPatternService, gapped_payments, test_accounts
):
    request = PaymentPatternRequest(
        account_id=test_accounts[0].id, category_id="utilities"
    )
    ...
    )  # Fixture creates 5 payments (skipping months 3, 4, and 5)
    assert any(
        "irregular" in note.lower() for note in analysis.notes
    )  # Check for "irregular" in notes


async def test_analyze_bill_payments_no_payments(
    payment_pattern_service: BillPaymentPatternService, db_session: AsyncSession
):
    """Test analyzing a bill with no payments"""
    result = await payment_pattern_service.analyze_bill_payments(
        999
    )  # Non-existent liability_id
...
async def test_filter_combination(
    ...
    for i in range(6):
        ...
        payment = Payment(
            ...
            category="utilities",
            liability_id=(
                test_bill.id if i < 3 else None
            ),  # Mix of liability and non-liability payments
        ...
        source = PaymentSource(
            payment=payment,
            account_id=(
                test_accounts[0].id if i % 2 == 0 else test_accounts[1].id
            ),  # Mix of credit and checking accounts
            amount=Decimal("100.00"),
        )
        db_session.add(source)
        payments.append(payment)
...
async def test_amount_statistics_edge_cases(
    ...
    assert (
        analysis.pattern_type == PatternType.SEASONAL
    )  # High amount variation with regular timing
...
async def test_borderline_regular_pattern(
    ...
    assert analysis.pattern_type == PatternType.REGULAR
    assert (
        0.6 <= analysis.confidence_score <= 0.875
    )  # Allow higher confidence for borderline cases
