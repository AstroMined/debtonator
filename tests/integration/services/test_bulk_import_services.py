...
def valid_liabilities_file(request):
    ...
    return UploadFile(
        filename="valid_liabilities.csv",
        file=file_handle
    )
...
def invalid_liabilities_file(request):
    ...
    request.addfinalizer(lambda: file_handle.close())
    return UploadFile(
        filename="invalid_liabilities.csv",
        file=file_handle
    )
...
@pytest.mark.asyncio
async def test_preview_invalid_liabilities_import(bulk_import_service, invalid_liabilities_file, request):
    """Test previewing invalid liabilities import"""
    preview = await bulk_import_service.preview_liabilities_import(invalid_liabilities_file)
    assert preview.total_records == 2
    assert len(preview.validation_errors) > 0
...
@pytest.mark.asyncio
async def test_preview_valid_income_import(bulk_import_service, valid_income_file, base_account, request):
...
@pytest.mark.asyncio
async def test_preview_invalid_income_import(bulk_import_service, invalid_income_file, request):
...
@pytest.mark.asyncio
async def test_import_invalid_liabilities(bulk_import_service, invalid_liabilities_file, request):
    """Test importing invalid liabilities"""
    result = await bulk_import_service.import_liabilities(invalid_liabilities_file, preview=False)
...
@pytest.mark.asyncio
async def test_import_valid_income(bulk_import_service, valid_income_file, base_account, request):
