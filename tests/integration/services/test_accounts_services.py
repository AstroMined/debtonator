...
@pytest.mark.asyncio
async def test_calculate_available_credit(
    db_session,
    base_credit_account
):
...
async def test_calculate_available_credit_non_credit_account(
    ...
    with pytest.raises(ValueError, match="Available credit calculation only available for credit accounts"):
        await service.calculate_available_credit(base_account.id)

@pytest.mark.asyncio
async def test_calculate_available_credit_account_not_found(
    db_session
):
