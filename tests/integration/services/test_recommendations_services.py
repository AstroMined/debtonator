...
async def test_payments(db_session: AsyncSession, test_bill, test_accounts):
    ...
    for i in range(6):
        ...
        if base_date.month + month_offset > 12:
            ...
            due_date = datetime(
                base_date.year + year_offset, month, 15, tzinfo=ZoneInfo("UTC")
            )
        else:
            due_date = datetime(
                base_date.year,
        ...
        print(
            f"Creating payment {i+1}: payment_date={payment_date}, due_date={due_date}, days_before={days_before_due}"
        )
...
async def test_get_bill_payment_recommendations(
    ...
    pattern = await recommendation_service.pattern_service.analyze_bill_payments(
        test_bill.id
    )
    print(f"\nPattern: {pattern}")
    ...
    )  # High confidence despite monthly day variations (28-31 days)
    assert (
        abs(pattern.frequency_metrics.average_days_between - 5) < 0.1
    )  # Close to 5 days
    ...
    assert (
        recommendation.historical_pattern_strength >= 0.8
    )  # Adjusted for monthly day variations
...
async def test_confidence_scoring(
    ...
    high_score = recommendation_service._confidence_to_decimal(ConfidenceLevel.HIGH)
    medium_score = recommendation_service._confidence_to_decimal(ConfidenceLevel.MEDIUM)
    low_score = recommendation_service._confidence_to_decimal(ConfidenceLevel.LOW)
...
async def test_credit_utilization_calculation(
    ...
    credit_accounts = [acc for acc in test_accounts if acc.type == "credit"]

    utilization = recommendation_service._calculate_credit_utilization(credit_accounts)
