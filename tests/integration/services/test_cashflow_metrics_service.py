...
@pytest.mark.asyncio
async def test_update_cashflow_deficits(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
    ...
    expected_min = min(test_cashflow_forecast.min_14_day, test_cashflow_forecast.min_30_day, 
                      test_cashflow_forecast.min_60_day, test_cashflow_forecast.min_90_day)
...
@pytest.mark.asyncio
async def test_update_cashflow_deficits_positive(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_update_cashflow_required_income(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_update_cashflow_required_income_custom_tax(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
    ...
    metrics_service.update_cashflow_required_income(test_cashflow_forecast, Decimal('0.75'))
...
@pytest.mark.asyncio
async def test_update_cashflow_hourly_rates(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_update_cashflow_all_calculations(test_cashflow_forecast: CashflowForecast, db_session: AsyncSession):
    ...
    assert test_cashflow_forecast.yearly_deficit == test_cashflow_forecast.daily_deficit * 365
    assert test_cashflow_forecast.required_income == test_cashflow_forecast.yearly_deficit / Decimal('0.8')
...
async def test_calculate_daily_deficit_method(db_session: AsyncSession):
    ...
    assert metrics_service.calculate_daily_deficit(Decimal('100.00'), 30) == Decimal('0.00')
    
    # Test negative amount
    assert metrics_service.calculate_daily_deficit(Decimal('-300.00'), 30) == Decimal('10.00')
    
    # Test zero amount
    assert metrics_service.calculate_daily_deficit(Decimal('0.00'), 30) == Decimal('0.00')
...
async def test_calculate_yearly_deficit_method(db_session: AsyncSession):
    ...
    assert metrics_service.calculate_yearly_deficit(Decimal('10.00')) == Decimal('3650.00')
...
async def test_calculate_required_income_method(db_session: AsyncSession):
    ...
    assert metrics_service.calculate_required_income(Decimal('1000.00')) == Decimal('1250.00')
    
    # Test with custom tax rate
    result = metrics_service.calculate_required_income(Decimal('1000.00'), Decimal('0.70'))
