...
from src.schemas.feature_flags import FeatureFlagType, FeatureFlagCreate
...
class TestFeatureFlagService:
    ...
    @pytest.fixture
    async def service(self, registry: FeatureFlagRegistry, repository: FeatureFlagRepository):
    ...
    async def test_initialize(self, registry: FeatureFlagRegistry, repository: FeatureFlagRepository, sample_flags_db):
    ...
    async def test_is_enabled_user_segment(self, service: FeatureFlagService):
        ...
        assert service.is_enabled("SEGMENT_FLAG", {"user_groups": ["regular", "beta"]}) is True
        
        # Regular user should not have it enabled
        assert service.is_enabled("SEGMENT_FLAG", {"user_groups": ["regular"]}) is False

    async def test_set_enabled(self, service: FeatureFlagService, repository: FeatureFlagRepository):
    ...
    async def test_create_flag(self, service: FeatureFlagService, repository: FeatureFlagRepository):
    ...
    async def test_delete_system_flag(self, service: FeatureFlagService, repository: FeatureFlagRepository):
    ...
    async def test_set_value(self, service: FeatureFlagService, repository: FeatureFlagRepository):
        ...
        assert result is True
        assert service.registry.get_flag("SEGMENT_VALUE_FLAG")["value"] == ["admin", "beta"]
