...
def irregular_income_data():
    ...
    base_date = date(2024, 1, 1)
    amounts = [
        "500.00", "750.00", "300.00", "1200.00",
        "450.00", "800.00", "600.00", "900.00"
    ]
...
async def test_source_statistics(db_session, weekly_income_data, monthly_income_data):
    ...
    monthly_stats = next(s for s in result.source_statistics if s.source == "Rental Income")
...
async def test_filtered_source_analysis(db_session, weekly_income_data, monthly_income_data):
