...
async def test_analyze_split_impact(
    ...
    checking_impact = next(
        impact for impact in result.account_impacts
    ...
    assert credit_impact.current_credit_utilization == Decimal("10.00")  # (500/5000) * 100
    assert credit_impact.projected_credit_utilization == Decimal("18.00")  # (900/5000) * 100
...
async def test_high_risk_scenario(
    ...
    assert any(
        "reducing the split amount" in rec
...
async def test_credit_utilization_risk(
    ...
    request = SplitImpactRequest(
        ...
        splits=[
            ...
            {"account_id": test_accounts[1].id, "amount": 4000}  # Would use 90% of credit
        ],
        analysis_period_days=30
    )
    ...
    assert any(
        "credit utilization" in rec.lower()
        for rec in result.recommendations
    )
