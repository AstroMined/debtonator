...
async def test_validate_update_reduce_installment_count(
    ...
    # Arrange
    data = {
        "installment_count": 1  # Can't reduce below the 2 already paid
    }
    
    # Act & Assert
    with pytest.raises(ValueError, match="Cannot reduce installment count below already paid"):
        await bnpl_service.validate_update(async_session, data, partial_paid_bnpl_account)
...
async def test_validate_update_invalid_installments_paid(
    ...
    # Arrange
    data = {
        "installments_paid": 5  # More than the total of 4
    }
    
    # Act & Assert
    with pytest.raises(ValueError, match="Installments paid cannot exceed total installment count"):
...
async def test_get_upcoming_payments(
    ...
    # Act
    payments = await bnpl_service.get_upcoming_payments(async_session, bnpl_account.id, 60)
...
async def test_update_bnpl_status_no_change(
    ...
    # Act
    updated_account = await bnpl_service.update_bnpl_status(async_session, bnpl_account.id)
...
async def test_update_bnpl_status_increment_payment(
    ...
    # Act
    updated_account = await bnpl_service.update_bnpl_status(async_session, past_due_bnpl_account.id)
    ...
    assert updated_account.next_payment_date > original_payment_date  # New date in future
