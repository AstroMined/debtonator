...
async def test_validate_create_missing_overdraft_limit(async_session: AsyncSession):
    ...
    # Act & Assert
    with pytest.raises(ValueError, match="Overdraft limit is required when overdraft protection is enabled"):
        await checking_service.validate_create(async_session, data)


@pytest.mark.asyncio
async def test_validate_update_exceeds_max_overdraft(async_session: AsyncSession, checking_account: CheckingAccount):
...
@pytest.mark.asyncio
async def test_update_overview(async_session: AsyncSession, checking_account: CheckingAccount):
...
@pytest.mark.asyncio
async def test_get_upcoming_payments_empty(async_session: AsyncSession, checking_account: CheckingAccount):
    """Test that get_upcoming_payments returns an empty list for checking accounts."""
    # Act
    result = await checking_service.get_upcoming_payments(async_session, checking_account.id, 14)
...
@pytest.mark.asyncio
async def test_account_persistence(async_session: AsyncSession, checking_account: CheckingAccount):
