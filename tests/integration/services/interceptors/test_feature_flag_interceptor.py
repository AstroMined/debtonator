...
class TestServiceInterceptor:
    ...
    async def test_interceptor_extracts_account_type_from_different_patterns(
        ...
        account_type = service_interceptor._extract_account_type(
            args=(),
            kwargs={"data": {"account_type": "checking"}}
        )
        ...
        account_type = service_interceptor._extract_account_type(
            args=(),
            kwargs={"schema": MockSchemaWithDump("ewa")}
        )
    ...
    async def test_interceptor_pattern_matching(self, service_interceptor):
        ...
        # Exact match
        assert service_interceptor._matches_pattern("test_method", "test_method") is True
        
        # Glob pattern match
        assert service_interceptor._matches_pattern("test_create_account", "test_*") is True
        assert service_interceptor._matches_pattern("create_checking_account", "create_*_account") is True
        assert service_interceptor._matches_pattern("update_ewa_account", "*_ewa_*") is True
        
        # Non-matches
        assert service_interceptor._matches_pattern("test_method", "other_method") is False
        assert service_interceptor._matches_pattern("test_method", "test_*_suffix") is False
