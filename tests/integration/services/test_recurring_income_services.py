...
async def test_recurring_income(db_session: AsyncSession, test_checking_account: Account) -> RecurringIncome:
...
async def test_create_recurring_income(db_session: AsyncSession, test_checking_account: Account):
...
async def test_create_income_from_recurring(
    db_session: AsyncSession,
    test_recurring_income_with_category: RecurringIncome
):
    """
    ...
    income_entry = service.create_income_from_recurring(
        test_recurring_income_with_category, 
        now.month, 
        now.year
    )
