...
async def test_get_payments(
    payment_service: PaymentService, test_payment: Payment, db_session: AsyncSession
):
...
async def test_create_payment(
    payment_service: PaymentService, test_checking_account, db_session: AsyncSession
):
    ...
    payment_create = PaymentCreate(
        ...
        sources=[
            PaymentSourceCreate(
                account_id=test_checking_account.id, amount=Decimal("150.00")
            )
        ],
    )
    ...
    assert payment.sources[0].account_id == test_checking_account.id


@pytest.mark.asyncio
async def test_create_payment_invalid_sources(
    payment_service: PaymentService, test_checking_account, db_session: AsyncSession
):
...
@pytest.mark.asyncio
async def test_create_payment_invalid_account(
    payment_service: PaymentService, db_session: AsyncSession
):
    ...
    payment_create = PaymentCreate(
        ...
        sources=[
            PaymentSourceCreate(
                account_id=99999, amount=Decimal("150.00")  # Non-existent account ID
            )
        ],
    )
...
async def test_update_payment_not_found(
    ...
    payment_update = PaymentUpdate(
        amount=Decimal("200.00"), category="Updated Category"
    )
...
async def test_update_payment_invalid_sources(
    ...
    with pytest.raises(
        ...
        await payment_service.update_payment(test_payment.id, payment_update)
...
async def test_update_payment_partial(
    ...
    payment_update = PaymentUpdate(category="Updated Category")

    # Update only the category
    updated_payment = await payment_service.update_payment(
        test_payment.id, payment_update
    )

    # Verify only category was updated
    assert updated_payment is not None
    assert updated_payment.category == "Updated Category"
    assert updated_payment.amount == test_payment.amount
    assert len(updated_payment.sources) == len(test_payment.sources)
...
async def test_update_payment(
    ...
    payment_update = PaymentUpdate(
        ...
        sources=[
            PaymentSourceCreate(
                account_id=test_checking_account.id, amount=Decimal("200.00")
            )
        ],
    )
...
async def test_delete_payment_with_sources(
    ...
    # Delete the payment
    result = await payment_service.delete_payment(test_payment.id)
    assert result is True

    # Verify payment and its sources were deleted
    deleted_payment = await payment_service.get_payment(test_payment.id)
...
async def test_get_payments_for_liability(
    ...
    payments = await payment_service.get_payments_for_liability(test_liability.id)

    # Verify payment is in results
    assert len(payments) == 1
    assert payments[0].id == test_payment.id
    assert payments[0].liability_id == test_liability.id
...
async def test_get_payments_for_account(
    ...
    payments = await payment_service.get_payments_for_account(test_checking_account.id)

    # Verify payment is in results
    assert len(payments) == 1
    assert payments[0].id == test_payment.id
    assert any(
        source.account_id == test_checking_account.id for source in payments[0].sources
    )
