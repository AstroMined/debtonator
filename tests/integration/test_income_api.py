...
@pytest.fixture
async def test_account(setup_db, client: AsyncClient, account_create_data):
    response = await client.post("/api/v1/accounts/", json=account_create_data)
    assert response.status_code == 201
    return response.json()

@pytest.mark.asyncio
async def test_create_income(setup_db, client: AsyncClient, test_account, income_create_data):
...
@pytest.mark.asyncio
async def test_get_income(setup_db, client: AsyncClient, test_account, income_create_data):
...
@pytest.mark.asyncio
async def test_update_income_deposit_status(setup_db, client: AsyncClient, test_account, income_create_data):
    # First create an income entry
    income_data = income_create_data(test_account["id"])
    create_response = await client.post("/api/v1/income/", json=income_data)
    assert create_response.status_code == 201
...
@pytest.mark.asyncio
async def test_get_all_income(setup_db, client: AsyncClient, test_account, income_create_data):
...
@pytest.mark.asyncio
async def test_create_income_validation(setup_db, client: AsyncClient, test_account):
...
@pytest.mark.asyncio
async def test_get_nonexistent_income(setup_db, client: AsyncClient):
    response = await client.get("/api/v1/income/999")
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_filter_income_by_account(setup_db, client: AsyncClient, test_account, income_create_data):
...
@pytest.mark.asyncio
async def test_update_income_amount_updates_balance(setup_db, client: AsyncClient, test_account, income_create_data):
