...
async def test_feature_flag_caching_demo(
    ...
    try:
        ...
        print(
            f"Operation still succeeded even though flag is disabled due to caching: {result2}"
        )
            ...
            if "repository" in requirements:
                ...
                if method_name in repository_reqs:
                    ...
                    if account_type in required_types:
                        ...
                        is_enabled = await self._feature_flag_service.is_enabled(
                            feature_name
                        )
