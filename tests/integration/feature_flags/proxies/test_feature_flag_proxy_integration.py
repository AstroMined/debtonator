...
async def test_proxy_account_type_whitelist(feature_flag_proxy, feature_flag_service):
    ...
    await feature_flag_service.set_account_types_whitelist(
        "TEST_FEATURE", ["allowed_type"]
    )
...
@pytest.mark.asyncio
async def test_proxy_with_banking_account_types(
    feature_flag_proxy, feature_flag_service
):
    """
    ...
    # Now it should work
    result = await feature_flag_proxy.create(
        {"account_type": "bnpl", "name": "Test BNPL"}
    )
