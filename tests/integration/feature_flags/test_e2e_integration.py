...
async def setup_test_feature_flag(
    ...
    if flag:
        await repository.update(
            flag_name, {"value": flag_value, "flag_type": "boolean"}
        )
    else:
...
async def test_e2e_flag(db_session: AsyncSession) -> FeatureFlag:
    ...
    flag = await setup_test_feature_flag(
        db_session, flag_name="TEST_E2E_FEATURE_FLAG", flag_value=True
    )
    return flag
...
async def test_repository_layer_enforcement(
    ...
    requirements = {"repository": {"create": ["bnpl"]}}
    ...
    assert (
        "repository" in str(excinfo.value).lower()
        or "create" in str(excinfo.value).lower()
    )
...
async def test_service_layer_enforcement(
    ...
    requirements = {"service": {"create_account": ["ewa"]}}
    await setup_test_feature_flag(
        db_session,
        flag_name=test_e2e_flag.name,
        flag_value=False,  # Disable the flag
...
async def test_api_layer_enforcement(
    client: AsyncClient, db_session: AsyncSession, test_e2e_flag: FeatureFlag
):
    ...
    requirements = {"api": {"/api/v1/accounts": ["payment_app"]}}
    await setup_test_feature_flag(
        db_session,
        flag_name=test_e2e_flag.name,
        flag_value=False,  # Disable the flag
...
async def test_cross_layer_requirements_propagation(
    ...
    requirements = {
        "repository": {"create": ["bnpl", "ewa", "payment_app"]},
    ...
    assert response.status_code in (
        403,
        500,
    )  # Either forbidden or internal server error
...
async def test_cache_invalidation(
    ...
    new_requirements = {"repository": {"create": ["ewa"]}}  # Changed from bnpl to ewa
    await setup_test_feature_flag(
        db_session,
        flag_name=test_e2e_flag.name,
        flag_value=False,  # Still disabled
    ...
    combined_requirements = {"repository": {"create": ["bnpl", "ewa"]}}
    await setup_test_feature_flag(
        db_session,
        flag_name=test_e2e_flag.name,
        flag_value=False,  # Still disabled
...
async def test_performance_overhead(
    ...
    ), f"Performance overhead too high: {overhead_percent:.2f}%"
