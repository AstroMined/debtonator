...
async def test_bulk_create_validation_failure(client: AsyncClient, test_liability, test_accounts):
    ...
    response = await client.post("/api/v1/bill-splits/bulk", json=payload)
    assert response.status_code == 200  # Still returns 200 as it's a valid request with validation errors
...
async def test_validate_bulk_operation_endpoint(client: AsyncClient, test_liability, test_accounts):
