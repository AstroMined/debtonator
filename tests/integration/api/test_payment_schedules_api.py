...
async def test_create_payment_schedule(client: AsyncClient, test_liability, test_account):
    schedule_data = {
        "liability_id": test_liability.id,
        "account_id": test_account.id,
...
async def test_get_schedules_by_liability(client: AsyncClient, test_schedule, test_liability):
    response = await client.get(
...
async def test_process_schedule(client: AsyncClient, test_schedule):
    response = await client.post(f"/api/v1/payment-schedules/{test_schedule.id}/process")
    assert response.status_code == 200
...
async def test_process_already_processed_schedule(client: AsyncClient, test_schedule, db_session):
    # First process
...
async def test_delete_processed_schedule(client: AsyncClient, test_schedule, db_session):
