...
async def test_create_income_category(client: AsyncClient):
    ...
    response = await client.post(
        ...
        json={"name": "Freelance", "description": "Income from freelance work"},
    )
...
async def test_list_income_categories(client: AsyncClient, sample_category):
...
async def test_get_income_category(client: AsyncClient, sample_category):
...
async def test_update_income_category(client: AsyncClient, sample_category):
    """Test updating an income category"""
    response = await client.put(
        f"/api/v1/income/categories/{sample_category.id}",
        json={"name": "Updated Salary", "description": "Updated description"},
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_delete_income_category(client: AsyncClient, sample_category):
...
async def test_update_nonexistent_category(client: AsyncClient):
    ...
    response = await client.put("/api/v1/income/categories/999", json={"name": "Test"})
    assert response.status_code == 404
...
async def test_create_duplicate_category(client: AsyncClient, sample_category):
    """Test creating a category with duplicate name"""
    response = await client.post(
        "/api/v1/income/categories",
        json={"name": "Salary", "description": "Duplicate name"},
    )
