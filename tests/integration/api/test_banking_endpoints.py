...
from tests.fixtures.models.account_types.banking import fixture_bnpl_account, fixture_checking_account
...
async def mock_account_service(monkeypatch, db_session: AsyncSession):
    ...
    monkeypatch.setattr("src.api.dependencies.services.get_account_service", override_get_account_service)
...
@pytest.mark.asyncio
async def test_get_banking_overview(test_app, mock_account_service, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_get_upcoming_payments(test_app, mock_account_service, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_create_banking_account(test_app, mock_account_service, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_update_bnpl_account_status(test_app, mock_account_service, db_session: AsyncSession):
    ...
    # Make the request
    response = test_app.post(f"/api/v1/banking/accounts/bnpl/{bnpl.id}/update-status", json=status_update)
...
@pytest.mark.asyncio
async def test_invalid_bnpl_account_status_update(test_app, mock_account_service, db_session: AsyncSession):
    ...
    response = test_app.post(f"/api/v1/banking/accounts/bnpl/{checking.id}/update-status", json=status_update)
