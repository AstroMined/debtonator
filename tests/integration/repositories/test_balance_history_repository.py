...
async def test_get_average_balance(db_session: AsyncSession):
    ...
    await repo.create(
        {"account_id": account.id, "balance": Decimal("2400.00"), "timestamp": now}
    )
...
async def test_add_balance_note(db_session: AsyncSession):
    ...
    updated_balance = await repo.add_balance_note(
        balance.id, "Reconciled with bank statement"
    )
    ...
    updated_note = await repo.add_balance_note(
        balance.id, "Updated after reconciliation"
    )
...
async def test_get_missing_days(db_session: AsyncSession):
    ...
    await repo.create(
        {"account_id": account.id, "balance": Decimal("2000.00"), "timestamp": day1}
    )

    day2 = today - timedelta(days=5)
    await repo.create(
        {"account_id": account.id, "balance": Decimal("2500.00"), "timestamp": day2}
    )

    day3 = today
    await repo.create(
        {"account_id": account.id, "balance": Decimal("3000.00"), "timestamp": day3}
    )
