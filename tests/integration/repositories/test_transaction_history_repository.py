...
from tests.helpers.schema_factories import create_account_schema, create_transaction_history_schema


@pytest_asyncio.fixture
async def transaction_history_repository(db_session: AsyncSession) -> TransactionHistoryRepository:
...
class TestTransactionHistoryRepository:
    ...
    async def test_update_transaction_history(
        ...
        assert result.transaction_type == test_transaction_history.transaction_type  # Unchanged
    ...
    async def test_get_with_account(
        ...
        result = await transaction_history_repository.get_with_account(test_transaction_history.id)
    ...
    async def test_get_transaction_count(
        ...
        counts = await transaction_history_repository.get_transaction_count(test_account.id)
        
        # 3. ASSERT: Verify the operation results
        assert TransactionType.CREDIT.value in counts
        assert TransactionType.DEBIT.value in counts
        assert counts[TransactionType.CREDIT.value] >= 3  # At least 3 credit transactions in fixture
        assert counts[TransactionType.DEBIT.value] >= 4  # At least 4 debit transactions in fixture
    ...
    async def test_get_net_change(
        ...
        net_change = await transaction_history_repository.get_net_change(test_account.id)
    ...
    async def test_get_transaction_patterns(
        ...
        patterns = await transaction_history_repository.get_transaction_patterns(test_account.id)
