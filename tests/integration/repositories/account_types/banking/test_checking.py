...
class TestCheckingAccountRepository:
    ...
    @pytest.fixture
    async def checking_repository(self, db_session: AsyncSession, feature_flag_service: FeatureFlagService) -> AccountRepository:
    ...
    async def test_get_checking_accounts_with_overdraft(
        self, checking_repository: AccountRepository, 
        test_checking_account: CheckingAccount,
    ...
    async def test_get_checking_accounts_with_international_features(
        ...
        for account in international_accounts:
            has_international_feature = (
                account.iban is not None or
                account.swift_bic is not None or
                account.sort_code is not None or
                account.branch_code is not None or
                account.account_format != "local"
            )
