...
class TestPaymentAppAccountAdvanced:
    ...
    async def test_get_payment_app_accounts_with_debit_cards(
        ...
        # 2. ACT: Call the specialized repository method
        accounts_with_debit_cards = await get_payment_app_accounts_with_debit_cards(db_session)
    ...
    async def test_get_payment_app_accounts_with_linked_accounts(
        ...
        # 2. ACT: Call the specialized repository method
        accounts_with_linked = await get_payment_app_accounts_with_linked_accounts(db_session)
    ...
    async def test_get_payment_app_accounts_with_direct_deposit(
        ...
        # 2. ACT: Call the specialized repository method
        accounts_with_direct_deposit = await get_payment_app_accounts_with_direct_deposit(db_session)
        
        # 3. ASSERT: Verify the results
        assert len(accounts_with_direct_deposit) >= 1
        assert all(account.supports_direct_deposit for account in accounts_with_direct_deposit)
    ...
    async def test_repository_has_specialized_methods(self, repository: AccountRepository):
        ...
        assert callable(getattr(repository, "get_payment_app_accounts_with_debit_cards"))
        
        assert hasattr(repository, "get_payment_app_accounts_with_linked_accounts")
        assert callable(getattr(repository, "get_payment_app_accounts_with_linked_accounts"))
        
        assert hasattr(repository, "get_payment_app_accounts_with_direct_deposit")
        assert callable(getattr(repository, "get_payment_app_accounts_with_direct_deposit"))
