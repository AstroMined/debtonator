...
async def test_create_typed_entity_invalid_type(polymorphic_test_repository):
    ...
    # 3. ACT & 4. ASSERT: Verify the method raises ValueError
    with pytest.raises(ValueError) as excinfo:
        await polymorphic_test_repository.create_typed_entity("invalid_type", entity_data)

    # Verify error message explains the issue
    assert "No model class registered for entity type 'invalid_type'" in str(excinfo.value)
...
async def test_update_typed_entity_invalid_type(polymorphic_test_repository):
    ...
    # 3. ACT & 4. ASSERT: Verify the method raises ValueError
    with pytest.raises(ValueError) as excinfo:
        await polymorphic_test_repository.update_typed_entity(1, "invalid_type", update_data)
...
@pytest.mark.asyncio
async def test_get_valid_fields_for_model(polymorphic_test_repository, test_type_registry):
    """
    ...
    type_a_fields = polymorphic_test_repository._get_valid_fields_for_model(TestTypeAModel)
    ...
    type_b_fields = polymorphic_test_repository._get_valid_fields_for_model(TestTypeBModel)
...
async def test_create_typed_entity_success(polymorphic_test_repository):
    ...
    entity = await polymorphic_test_repository.create_typed_entity("type_a", entity_data)
...
async def test_create_typed_entity_with_required_field(polymorphic_test_repository):
    ...
    # 3. ACT: Create entity through repository
    entity = await polymorphic_test_repository.create_typed_entity("type_b", entity_data)
...
async def test_field_filtering_for_required_fields(polymorphic_test_repository):
    ...
    assert updated_entity.b_field == "Required Value"  # b_field preserved original value
