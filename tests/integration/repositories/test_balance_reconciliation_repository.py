...
from tests.helpers.schema_factories import create_account_schema, create_balance_reconciliation_schema


@pytest_asyncio.fixture
async def balance_reconciliation_repository(db_session: AsyncSession) -> BalanceReconciliationRepository:
...
class TestBalanceReconciliationRepository:
    """
    Tests for the BalanceReconciliationRepository.
    ...
    async def test_update_balance_reconciliation(
        ...
        adjustment_amount = Decimal("1075.25") - test_balance_reconciliation.previous_balance
    ...
    async def test_get_by_account(
        ...
        results = await balance_reconciliation_repository.get_by_account(test_account.id)
    ...
    async def test_get_with_account(
        ...
        result = await balance_reconciliation_repository.get_with_account(test_balance_reconciliation.id)
    ...
    async def test_get_most_recent(
        ...
        result = await balance_reconciliation_repository.get_most_recent(test_account.id)
    ...
    async def test_get_total_adjustment_amount(
        ...
        total = await balance_reconciliation_repository.get_total_adjustment_amount(test_account.id)
    ...
    async def test_get_adjustment_count_by_reason(
        ...
        result = await balance_reconciliation_repository.get_adjustment_count_by_reason(test_account.id)
    ...
    async def test_get_reconciliation_frequency(
        ...
        frequency = await balance_reconciliation_repository.get_reconciliation_frequency(test_account.id)
    ...
    async def test_validation_error_handling(self):
        ...
        try:
            invalid_schema = BalanceReconciliationCreate(
                account_id=999,
                previous_balance=Decimal("1000.00"),
                new_balance=Decimal("1000.00"),  # Same as previous, so adjustment would be 0
