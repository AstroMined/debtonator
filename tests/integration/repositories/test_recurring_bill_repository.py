...
async def test_get_by_day_of_month(db_session: AsyncSession):
    ...
    assert not any(bill.id in [day10_bill1.id, day10_bill2.id, day15_bill.id] 
...
async def test_get_with_relationships(db_session: AsyncSession):
    ...
    bill_with_account = await repo.get_with_relationships(
        bill.id, include_account=True
    )
...
async def test_get_monthly_total(db_session: AsyncSession):
    ...
    # Assert
    assert new_total >= initial_total + Decimal("300.00")  # Active bills should be included
