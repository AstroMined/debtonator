...
async def test_is_ancestor_of(db_session: AsyncSession):
    ...
    # Test is_ancestor_of
    is_grandparent_ancestor_of_child = await repo.is_ancestor_of(grandparent.id, child.id)
...
async def test_get_category_with_bill_count(db_session: AsyncSession):
    ...
    category_with_count, bill_count = await category_repo.get_category_with_bill_count(category.id)
...
async def test_get_categories_with_bill_counts(db_session: AsyncSession):
    ...
    category1_result = next((item for item in categories_with_counts if item[0].id == category1.id), None)
    category2_result = next((item for item in categories_with_counts if item[0].id == category2.id), None)
    category3_result = next((item for item in categories_with_counts if item[0].id == category3.id), None)
