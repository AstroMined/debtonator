...
class TestBillSplitsWithAccountTypes:
    ...
    @pytest.fixture
    async def bill_split_repository(
        self, db_session: AsyncSession, feature_flag_service=None
    ) -> BillSplitRepository:
        """Create a bill split repository for testing."""
        return BillSplitRepository(db_session, feature_flag_service)

    @pytest.fixture
    async def bill_with_checking_primary(
        self, db_session: AsyncSession, test_checking_account: CheckingAccount
    ) -> dict:
    ...
    async def test_create_bill_split_with_different_account_types(
        ...
        primary_split = next(
            s for s in all_splits if s.account_id == primary_account_id
        )
        credit_split = next(
            s
            for s in all_splits
            if s.account_id == test_accounts_for_splits["credit"].id
        )
        savings_split = next(
            s
            for s in all_splits
            if s.account_id == test_accounts_for_splits["savings"].id
        )
    ...
    async def test_transaction_rollback_on_validation_failure(
        ...
        with pytest.raises(Exception):  # Could be IntegrityError or similar
            await bill_split_repository.create_bill_splits(
                bill_id, invalid_account_splits
            )
    ...
    async def test_creating_primary_account_split_automatically(
        ...
        primary_splits = [
            s for s in created_splits if s.account_id == primary_account_id
        ]
