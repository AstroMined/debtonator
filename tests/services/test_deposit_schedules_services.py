...
async def test_create_deposit_schedule(
    ...
    success, error, schedule = await deposit_schedule_service.create_deposit_schedule(schedule_data)
...
async def test_get_deposit_schedule(
    ...
    schedule = await deposit_schedule_service.get_deposit_schedule(test_deposit_schedule.id)
    assert schedule is not None
...
async def test_update_deposit_schedule(
    ...
    update_data = DepositScheduleUpdate(
        amount=Decimal("500.00"),
        status="completed"
    )
    success, error, updated_schedule = await deposit_schedule_service.update_deposit_schedule(
...
async def test_delete_deposit_schedule(
    ...
    success, error = await deposit_schedule_service.delete_deposit_schedule(test_deposit_schedule.id)
    assert success is True
    assert error is None
    
    # Verify deletion
    deleted_schedule = await deposit_schedule_service.get_deposit_schedule(test_deposit_schedule.id)
...
async def test_get_pending_deposits(
    ...
    pending_deposits = await deposit_schedule_service.get_pending_deposits(test_account.id)
