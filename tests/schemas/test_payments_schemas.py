...
def test_payment_source_base_validation():
    ...
    with pytest.raises(ValidationError, match="Amount must have at most 2 decimal places"):
...
def test_payment_create_validation():
    ...
    # Invalid: no sources
    with pytest.raises(ValidationError, match="At least one payment source is required"):
    ...
    with pytest.raises(ValidationError, match="Sum of payment sources .* must equal payment amount"):
    ...
    with pytest.raises(ValidationError, match="Duplicate account IDs in payment sources"):
