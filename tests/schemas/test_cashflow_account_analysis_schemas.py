...
def test_transfer_pattern_valid():
    ...
    assert pattern.category_distribution == {"Bills": Decimal("0.7"), "Savings": Decimal("0.3")}
...
def test_account_usage_pattern_valid():
    ...
    usage = AccountUsagePattern(
        ...
        category_preferences={"Groceries": Decimal("0.4"), "Shopping": Decimal("0.3"), "Bills": Decimal("0.3")},
    ...
    assert usage.category_preferences == {"Groceries": Decimal("0.4"), "Shopping": Decimal("0.3"), "Bills": Decimal("0.3")}
...
def test_balance_distribution_valid():
    ...
    assert distribution.typical_balance_range == (Decimal("2000.00"), Decimal("3000.00"))
...
def test_transfer_pattern_field_validation():
    ...
    with pytest.raises(ValidationError, match="Input should be less than or equal to 31"):
        TransferPattern(
            source_account_id=1,
            target_account_id=2,
            average_amount=Decimal("500.00"),
...
def test_account_usage_pattern_validation():
    ...
    with pytest.raises(ValidationError, match="List should have at most 100 items"):
        AccountUsagePattern(
            ...
            primary_use="Daily Expenses",
            average_transaction_size=Decimal("75.50"),
            common_merchants=["Merchant" + str(i) for i in range(101)],  # 101 items, exceeds max length
            peak_usage_days=[1, 15],
            category_preferences={"Groceries": Decimal("0.5"), "Bills": Decimal("0.5")},
            utilization_rate=Decimal("0.65")
        )
