...
def test_custom_forecast_result_valid():
    ...
    result = CustomForecastResult(
        ...
        risk_factors={"Unexpected Expenses": Decimal("0.3"), "Income Variability": Decimal("0.2")}
    )
    ...
    assert result.contributing_factors == {"Income": Decimal("0.6"), "Bills": Decimal("0.4")}
    assert result.risk_factors == {"Unexpected Expenses": Decimal("0.3"), "Income Variability": Decimal("0.2")}
...
def test_account_forecast_result_valid():
    ...
    result = AccountForecastResult(
        ...
        contributing_transactions=[
            {"amount": Decimal("500.00"), "category": "Salary", "confidence": Decimal("0.95")},
            {"amount": Decimal("-300.00"), "category": "Bills", "confidence": Decimal("0.90")}
        ],
...
def test_account_forecast_response_valid():
    ...
    result1 = AccountForecastResult(
        ...
        confidence_score=Decimal("0.85"),
        contributing_transactions=[
            {"amount": Decimal("500.00"), "category": "Salary"}
        ]
    )
