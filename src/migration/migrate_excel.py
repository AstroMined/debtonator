...
async def migrate_data(excel_path: str) -> None:
    ...
    async with async_session() as session:
        ...
        print(
            f"Income entries imported: {income_stats['success']}/{income_stats['total']}"
        )
        ...
        print(
            f"Cashflow entries imported: {cashflow_stats['success']}/{cashflow_stats['total']}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate Excel data to SQLite database"
    )
    parser.add_argument(
        "excel_path", help="Path to the Excel file containing historical data"
    )
