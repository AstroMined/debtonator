    ...
    def extract_bills(self) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            bill = {
                ...
                "paid": bool(row["Paid?"]),
                "amex_amount": (
                    Decimal(str(row["AMEX"])) if pd.notna(row["AMEX"]) else None
                ),
                "unlimited_amount": (
                    Decimal(str(row["Unlimited"]))
                    if pd.notna(row["Unlimited"])
                    else None
                ),
                "ufcu_amount": (
                    Decimal(str(row["UFCU"])) if pd.notna(row["UFCU"]) else None
                ),
        ...
        for idx, item in enumerate(data):
            ...
            for field in required_fields:
                if field not in item or item[field] is None:
                    errors.append(
                        f"Missing required field '{field}' in {data_type} record {idx + 1}"
                    )
            ...
            for field in numeric_fields.get(data_type, []):
                if field in item and item[field] is not None:
                    try:
                        ...
                        errors.append(
                            f"Invalid numeric value for '{field}' in {data_type} record {idx + 1}"
                        )
