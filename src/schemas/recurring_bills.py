...
class RecurringBillUpdate(BaseSchemaValidator):
    ...
    day_of_month: Optional[int] = Field(
        ...
        ge=1, 
        le=31, 
        description="Updated day of the month when the bill is due"
    )
    account_id: Optional[int] = Field(
        None,
        description="Updated ID of the account associated with this bill"
    )
    category_id: Optional[int] = Field(
        None,
        description="Updated ID of the category associated with this bill"
    )
    auto_pay: Optional[bool] = Field(
        None,
        description="Updated automatic payment setting"
    )
...
class RecurringBillResponse(RecurringBillBase):
    ...
    active: bool = Field(..., description="Whether the recurring bill is currently active")
    created_at: datetime = Field(..., description="Date and time when the record was created in UTC timezone")
    updated_at: datetime = Field(..., description="Date and time when the record was last updated in UTC timezone")
...
class GenerateBillsRequest(BaseSchemaValidator):
    ...
    month: int = Field(
        ...
        ge=1, 
        le=12, 
        description="Month for which to generate bills (1-12)"
    )
    year: int = Field(
        ..., 
        ge=2000, 
        le=3000, 
        description="Year for which to generate bills"
    )
