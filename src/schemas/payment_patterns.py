...
class FrequencyMetrics(BaseSchemaValidator):
    ...
    average_days_between: float = Field(
        ..., description="Average number of days between payments", ge=0
    )
    std_dev_days: float = Field(
        ..., description="Standard deviation of days between payments", ge=0
    )
    min_days: int = Field(..., description="Minimum days between payments", ge=0)
    max_days: int = Field(..., description="Maximum days between payments", ge=0)
...
class AmountStatistics(BaseSchemaValidator):
    ...
    average_amount: MoneyDecimal = Field(..., description="Average payment amount")
    std_dev_amount: MoneyDecimal = Field(
        ..., ge=0, description="Standard deviation of payment amounts"
    )
    min_amount: MoneyDecimal = Field(..., description="Minimum payment amount")
    max_amount: MoneyDecimal = Field(..., description="Maximum payment amount")
    total_amount: MoneyDecimal = Field(..., description="Total amount of all payments")
...
class SeasonalMetrics(BaseSchemaValidator):
    ...
    avg_days_before_due: float = Field(
        ..., description="Average days before due date", ge=0
    )
    std_dev_days: float = Field(
        ..., description="Standard deviation of days before due date", ge=0
    )
    sample_size: int = Field(..., description="Number of payments in this season", gt=0)
...
class PaymentPatternAnalysis(BaseSchemaValidator):
    ...
    notes: Optional[List[str]] = Field(
        None, description="Additional notes or observations about the pattern"
    )
    seasonal_metrics: Optional[Dict[int, SeasonalMetrics]] = Field(
        None, description="Monthly or seasonal metrics, keyed by month number (1-12)"
    )
...
class PaymentPatternRequest(BaseSchemaValidator):
    ...
    account_id: Optional[int] = Field(
        None, description="Optional account ID to filter payments", gt=0
    )
    category_id: Optional[str] = Field(
        None, description="Optional category ID to filter payments", max_length=100
    )
    ...
    liability_id: Optional[int] = Field(
        None, description="Optional liability ID to filter payments", gt=0
    )
