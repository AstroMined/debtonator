...
class PaymentSourceInDB(PaymentSourceBase):
    ...
    id: int = Field(..., description="Unique identifier for the payment source")
    payment_id: int = Field(..., description="ID of the parent payment")
...
class PaymentBase(BaseSchemaValidator):
    ...
    amount: MoneyDecimal = Field(
        ..., gt=Decimal("0"), description="Total payment amount"
    )
    payment_date: datetime = Field(
        ..., description="Date and time of payment (UTC timezone)"
    )
    description: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Optional payment description"
    )
    category: str = Field(
        ...,
        min_length=1,
...
def validate_payment_sources(
    sources: List[PaymentSourceCreate], payment_amount: Optional[Decimal] = None
...
class PaymentUpdate(BaseSchemaValidator):
    ...
    amount: Optional[MoneyDecimal] = Field(
        None, gt=Decimal("0"), description="Total payment amount"
    )
    payment_date: Optional[datetime] = Field(
        None, description="Date and time of payment (UTC timezone)"
    )
    description: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Optional payment description"
    )
    ...
    sources: Optional[List[PaymentSourceCreate]] = Field(
        None, description="Payment sources (accounts from which the payment is drawn)"
    )
    ...
    def validate_sources_update(
        cls, sources: Optional[List[PaymentSourceCreate]], info: Any
    ) -> Optional[List[PaymentSourceCreate]]:
        """
...
class PaymentInDB(PaymentBase):
    ...
    liability_id: Optional[int] = Field(
        None, description="ID of the associated liability, if any"
    )
    income_id: Optional[int] = Field(
        None, description="ID of the associated income, if any"
    )
    created_at: datetime = Field(
    ...
    sources: List[PaymentSourceResponse] = Field(
        default_factory=list, description="Payment sources associated with this payment"
    )
