...
class IncomePattern(BaseSchemaValidator):
    ...
    next_predicted: Optional[datetime] = Field(
        None,
        description="Predicted date of the next occurrence (UTC timezone)"
    )
...
class SeasonalityMetrics(BaseSchemaValidator):
    ...
    period: PeriodType = Field(
        ...,
        description="Period type for seasonality analysis"
    )
...
class IncomeTrendsAnalysis(BaseSchemaValidator):
    ...
    patterns: List[IncomePattern] = Field(
        ..., 
        min_items=1,
        description="List of identified income patterns"
    )
    seasonality: Optional[SeasonalityMetrics] = Field(
        None,
        description="Seasonal patterns if detected"
    )
    source_statistics: List[SourceStatistics] = Field(
        ..., 
        min_items=1,
        description="Statistical analysis per income source"
    )
