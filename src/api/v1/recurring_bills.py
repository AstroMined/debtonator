...
async def get_recurring_bills(
    ...
    return await service.get_recurring_bills(skip=skip, limit=limit, active_only=active_only)
...
async def create_recurring_bill(
    recurring_bill: RecurringBillCreate,
    db: AsyncSession = Depends(get_db)
):
...
async def update_recurring_bill(
    ...
    service = RecurringBillService(db)
    updated_bill = await service.update_recurring_bill(recurring_bill_id, recurring_bill)
    if not updated_bill:
        raise HTTPException(status_code=404, detail="Recurring bill not found")
    return updated_bill
