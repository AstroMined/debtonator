...
class PaymentSourceRepository(BaseRepository[PaymentSource, int]):
    ...
    async def get_sources_for_payment(
        ...
        query = (
            ...
            .where(PaymentSource.payment_id == payment_id)
        )
    ...
    async def get_total_amount_by_account(
        self, 
        account_id: int
    ) -> Decimal:
        """
        ...
        """
        query = (
            select(func.sum(PaymentSource.amount))
            .where(PaymentSource.account_id == account_id)
        )
    ...
    async def delete_sources_for_payment(
        self, 
        payment_id: int
    ) -> int:
        """
