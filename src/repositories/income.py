...
class IncomeRepository(BaseRepository[Income, int]):
    ...
    async def get_by_source(self, source: str) -> List[Income]:
        ...
        query = (
            select(Income)
            .where(Income.source.ilike(f"%{source}%"))
        )
    ...
    async def get_undeposited_income(self, account_id: Optional[int] = None) -> List[Income]:
        """
    ...
    async def get_income_statistics_by_period(
        ...
        avg_query = (
            select(func.avg(Income.amount))
            .where(and_(*conditions))
        )
        
        # Query for minimum amount
        min_query = (
            select(func.min(Income.amount))
            .where(and_(*conditions))
        )
        
        # Query for maximum amount
        max_query = (
            select(func.max(Income.amount))
            .where(and_(*conditions))
        )
