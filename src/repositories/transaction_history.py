...
class TransactionHistoryRepository(BaseRepository[TransactionHistory, int]):
    ...
    async def get_monthly_totals(
        ...
        query = (
            select(
                func.date_trunc('month', TransactionHistory.transaction_date).label('month'),
                TransactionHistory.transaction_type,
    ...
    async def get_transaction_patterns(
        ...
        for key, data in patterns.items():
            ...
            if data['count'] > 1:
                ...
                count_factor = min(data['count'] / 10, 1.0)  # Max 1.0 at 10+ occurrences
                
                # Check for consistency in intervals
                if day_diffs:
                    std_dev = (sum((x - (sum(day_diffs) / len(day_diffs))) ** 2 for x in day_diffs) / len(day_diffs)) ** 0.5
                    consistency_factor = max(0, 1.0 - (std_dev / 30))  # Lower std_dev = higher consistency
                else:
                ...
                if data['average_days_between'] > 0:
                    if 25 <= data['average_days_between'] <= 35:
                        ...
                        data['pattern_type'] = f'Every {round(data["average_days_between"])} days'
