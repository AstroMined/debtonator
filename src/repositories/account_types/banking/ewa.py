...
async def get_ewa_accounts_approaching_payday(session: AsyncSession, days: int = 7):
...
async def get_ewa_accounts_by_provider(session: AsyncSession, provider: str):
    ...
    stmt = select(EWAAccount).where(
        EWAAccount.provider == provider, EWAAccount.is_closed == False
    )
    result = await session.execute(stmt)
    return result.scalars().all()
...
async def get_ewa_accounts_with_no_transaction_fee(session: AsyncSession):
    ...
    stmt = select(EWAAccount).where(
        (EWAAccount.per_transaction_fee == 0)
        | (EWAAccount.per_transaction_fee == None),
        EWAAccount.is_closed == False,
    )
    result = await session.execute(stmt)
    return result.scalars().all()
