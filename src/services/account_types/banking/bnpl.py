...
async def validate_create(session: AsyncSession, data: Dict[str, Any]) -> None:
    ...
    if (
        "installments_paid" in data
        and data["installments_paid"] > data["installment_count"]
    ):
...
async def validate_update(
    ...
    if "installment_count" in data:
        ...
        if data["installment_count"] < getattr(
            existing_account, "installments_paid", 0
        ):
            raise ValueError(
                "Cannot reduce installment count below already paid installments"
            )
    ...
    if "installments_paid" in data:
        ...
        installment_count = data.get(
            "installment_count", getattr(existing_account, "installment_count", 0)
        )
    ...
    if "current_balance" in data and "installment_amount" not in data:
        ...
        installments_paid = data.get(
            "installments_paid", getattr(existing_account, "installments_paid", 0)
        )
        installment_amount = getattr(
            existing_account, "installment_amount", Decimal("0")
        )
...
async def get_upcoming_payments(
    ...
    if (
        ...
        and account.next_payment_date
        and now <= account.next_payment_date <= end_date
    ):
        ...
        upcoming_payments.append(
            {
                ...
                "remaining_installments": account.installment_count
...
async def update_bnpl_status(session: AsyncSession, account_id: int) -> Optional[Any]:
    ...
    if (
        ...
        and account.next_payment_date < now
        and account.installments_paid < account.installment_count
    ):
        ...
        if account.payment_frequency == "weekly":
            ...
            next_payment_date = account.next_payment_date.replace(
                ...
                year=account.next_payment_date.year
                + (account.next_payment_date.month == 12),
            )
        else:
            # Default to biweekly
            next_payment_date = account.next_payment_date + timedelta(days=14)
...
async def prepare_account_data(
    ...
    if (
        "current_balance" not in account_data
        and "installment_amount" in account_data
        and "installment_count" in account_data
    ):
        remaining = account_data["installment_count"] - account_data.get(
            "installments_paid", 0
        )
    ...
    if "next_payment_date" not in account_data:
        ...
        account_data["next_payment_date"] = datetime.now(timezone.utc) + timedelta(
            days=30
        )
