...
class BulkImportService:
    def __init__(self, liability_service: LiabilityService, income_service: IncomeService, category_service: CategoryService):
        self.liability_service = liability_service
        self.income_service = income_service
        self.category_service = category_service

    async def validate_liability_record(self, record: dict, row_num: int) -> Tuple[Optional[LiabilityCreate], Optional[ImportError]]:
        ...
        try:
            ...
            try:
                ...
                return None, ImportError(
                    row=row_num,
                    field="date",
                    message="Invalid date format or values"
                )
            ...
            try:
                ...
                category = await self.category_service.get_category_by_name(account_name)
                
                # If category doesn't exist, try to get Uncategorized
                if not category and account_name != "Uncategorized":
                    category = await self.category_service.get_category_by_name("Uncategorized")
            ...
            liability_data = {
                ...
                "primary_account_id": int(record.get("primary_account_id", "1"))  # Default to account ID 1
            }
    ...
    async def validate_income_record(self, record: dict, row_num: int) -> Tuple[Optional[IncomeCreate], Optional[ImportError]]:
    ...
    async def preview_liabilities_import(self, file: UploadFile) -> BulkImportPreview:
        ...
        for i, record in enumerate(records, 1):
            ...
            required_fields = ["bill_name", "amount", "month", "day_of_month", "account_name"]
    ...
    async def import_liabilities(self, file: UploadFile, preview: bool = True) -> Union[BulkImportResponse, BulkImportPreview]:
        ...
        for i, liability in enumerate(preview_result.records, 1):
            try:
                ...
                created_liability = await self.liability_service.create_liability(liability)
