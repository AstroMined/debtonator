...
class StatementService(BaseService):
    ...
    async def get_upcoming_statements(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        """
        statement_repo = await self._get_repository(StatementHistoryRepository)
        statement_account_pairs = await statement_repo.get_upcoming_statements_with_accounts(days)
        ...
        for statement, account in statement_account_pairs:
            result.append({
                "statement": statement,
                "account": account,
                "days_until_due": (statement.due_date - utc_now()).days if statement.due_date else None,
            })
    ...
    async def get_total_minimum_payments_due(
        ...
        statement_repo = await self._get_repository(StatementHistoryRepository)
        return await statement_repo.get_total_minimum_payments_due(due_date_start, due_date_end)
    ...
    async def get_statement_by_date(
        ...
        statement_repo = await self._get_repository(StatementHistoryRepository)
        return await statement_repo.get_statement_by_date(account_id, statement_date.date())
