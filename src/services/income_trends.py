...
class IncomeTrendsService:
    ...
    async def analyze_trends(self, request: IncomeTrendsRequest) -> IncomeTrendsAnalysis:
        ...
        for source, records in source_records.items():
            ...
            if pattern.frequency == "irregular" or pattern.confidence_score >= request.min_confidence:
    ...
    async def _analyze_source_pattern(self, source: str, records: List[Income]) -> IncomePattern:
        ...
        interval_std = Decimal(str(statistics.stdev(intervals) if len(intervals) > 1 else float('inf')))
    ...
    def _determine_frequency(self, avg_interval: Decimal, interval_std: Decimal) -> Tuple[str, Decimal]:
        ...
        for pattern, expected in patterns.items():
            ...
            if deviation <= Decimal('2') and interval_std <= Decimal('2'):
                confidence = Decimal('1.0') - (deviation / Decimal('4')) - (interval_std / Decimal('4'))
    ...
    async def _calculate_source_statistics(self, source: str, records: List[Income]) -> SourceStatistics:
        ...
        return SourceStatistics(
            ...
            standard_deviation=Decimal(str(statistics.stdev(amounts) if len(amounts) > 1 else 0)),
    ...
    def _calculate_reliability_score(self, records: List[Income]) -> Decimal:
        ...
        cv_interval = std_interval / mean_interval if mean_interval > 0 else Decimal('inf')
    ...
    async def _analyze_seasonality(self, records: List[Income]) -> Optional[SeasonalityMetrics]:
        ...
        peak_months = [
            month for month, amount in monthly_averages.items()
    ...
    def _calculate_overall_predictability(
        ...
        result = (pattern_score * pattern_weight) + (reliability_score * reliability_weight)
        return float(result)
